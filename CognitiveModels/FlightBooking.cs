...
namespace Microsoft.BotBuilderSamples
{
    ...
    public class FlightBooking : IRecognizerConvert
    {
        ...
        public class CluEntities
        {
            ...
            public CluEntity[] GetProjectList() => Entities.Where(e => e.Category == "project").ToArray();
            public CluEntity[] GetSuiteList() => Entities.Where(e => e.Category == "suite").ToArray();
            public CluEntity[] GetStatusList() => Entities.Where(e => e.Category == "status").ToArray();
            public CluEntity[] GetDayList() => Entities.Where(e => e.Category == "day").ToArray();
            public CluEntity[] GetItemList() => Entities.Where(e => e.Category == "item").ToArray();

            public string GetProject() => GetProjectList().FirstOrDefault()?.Text;
            public string GetSuite() => GetSuiteList().FirstOrDefault()?.Text;
            public string GetStatus() => GetStatusList().FirstOrDefault()?.Text;
            public string GetDay() => GetDayList().FirstOrDefault()?.Text;
            public string GetItem() => GetItemList().FirstOrDefault()?.Text;
        }
    }
}
